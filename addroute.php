...
if (isset($_POST['addstops'])) {
  ...
  for ($i=0; $i <$stops ; $i++) { 
    $train=$_POST[$x];
    $query="INSERT into `trains_route` values ('$trainno','$train','$stopno')";
  ...
  <div class="well" align="center">
    ...
    ?>
      <div class="form-group">
        <label for="" class="control-label">Stop No:<?php echo $stopno ?></label>
        <input type="text" name="<?php echo $stopno ?>" class="form-control" style="width:400px">
      </div>
      <?php
      $stopno=$stopno+1;
    }
    ?>
    <div class="form-group">
      <input type="submit" value="Add Stops" name="addstops" class="btn btn-success">
    </div>
    </form>
  </div>
  </div>
